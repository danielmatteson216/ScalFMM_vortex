...
#pragma omp for nowait schedule(dynamic, userChunkSize)
                for( int idxCell = nbCellsToSkip ; idxCell < nbCellsForThreads ; ++idxCell){
                    ...
                    for(int idxChild = 0 ; idxChild < 8 ; ++idxChild){
                        source_children[idxChild] = nullptr;
                    ...
                    if(nbChildWithSrc){
                        ...
                        multipole_t* const parent_multipole
                            = &(currentCell->getMultipoleData());
                        const symbolic_data_t* const parent_symbolic = currentCell;
                    ...
                    if(hasToReceive){
                        ...
                        if(hasTgtChild){
                            ...
                            if(nbTgtChild){
                                ...
                                std::transform(potentialChild, potentialChild+8,
                    ...
                    if( iterArray[idxCell].getCurrentCell()->hasTargetsChild() ){
                        ...
                        for(int idxChild = 0 ; idxChild < 8 ; ++idxChild){
                            if(realChild[idxChild] && realChild[idxChild]->hasTargetsChild()){
                                ...
                                target_children[idxChild] = nullptr;
                            }
                        }
                        if(nbTgtChild){
                            local_expansion_t* const parent_expansion
