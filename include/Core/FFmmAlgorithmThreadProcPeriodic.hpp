...
#pragma omp for nowait  schedule(dynamic, userChunkSize)
  ...
  /////////////////////////////////////////////////////////////////////////////
  struct LeafData{
    FTreeCoordinate coord;     ///< the 3d morton index of the cell that contains the leaf
    CellClass*      cell;      ///<  a pointer on the cell
    ContainerClass* targets;   ///<  a pointer on the target particles
    ContainerClass* sources;   ///<  a pointer on the source particles
  };
...
//shared(leafsNeedOtherData)
#pragma omp parallel  num_threads(MaxThreads)
    {
#pragma omp master
      {  // MUST WAIT to fill leafsNeedOther pourquoi pas single (?)
        if(p2pEnabled){
            // Copy leafs in sequential
            // The goal is to transform the while loop in a for loop
            {
              typename OctreeClass::Iterator octreeIterator(tree);
              octreeIterator.gotoBottomLeft();
              int idxLeaf = 0;
            ...
            for(int idxLeaf = 0 ; idxLeaf < this->numberOfLeafs ; ++idxLeaf){
                ...
                const int neighCount = this->getNeighborsIndexesPeriodic(iterArray[idxLeaf].getCurrentGlobalCoordinate(),
                                                                         limite,
                ...
                for(int idxNeigh = 0 ; idxNeigh < neighCount ; ++idxNeigh){
                    ...
                    if(indexesNeighbors[idxNeigh] < (intervals[idProcess].leftIndex) ||
    ...
    if(p2pEnabled){
       ...
       */
        ...
        FMpi::MpiAssert( MPI_Allgather( partsToSend,      nbProcess, FMpi::GetType(*partsToSend),
                                        globalReceiveMap, nbProcess, FMpi::GetType(*partsToSend),
        ...
        // set leaves in otherP2Ptree  (LET-local Tree)
        for(int idxProc = 0 ; idxProc < nbProcess ; ++idxProc){
            if(globalReceiveMap[idxProc * nbProcess + idProcess]){ //if idxProc has sth for me.
...
#pragma omp single // MUST WAIT!
    {
      ...
      for(int idxLeaf = 0 ; idxLeaf < this->numberOfLeafs ; ++idxLeaf, ++idxInArray){
          ...
          auto &  pos = startPosAtShape[shapePosition] ;
          leafsDataArray[pos].coord   = myLeafs[idxInArray].getCurrentGlobalCoordinate();
          leafsDataArray[pos].cell    = myLeafs[idxInArray].getCurrentCell();
          leafsDataArray[pos].targets = myLeafs[idxInArray].getCurrentListTargets();
          leafsDataArray[pos].sources = myLeafs[idxInArray].getCurrentListSrc();
          if( leafsNeedOther.get(idxLeaf) ) {
              leafsNeedOtherData.push(leafsDataArray[pos]);
            }
...
#pragma omp task default(none) firstprivate(nbLeavesInTask,idxLeafs)
		{
		  ...
		  for(int idxTaskLeaf = idxLeafs ; idxTaskLeaf < (idxLeafs + nbLeavesInTask) ; ++idxTaskLeaf){
		      ...
		      // compute P2P operator
		      if(p2pEnabled){
			  // need the current particles and neighbors particles
			  //  we get the neighbors on th current process (local tree)
			  ...
			  //     For Periodic cells
			  //          1) move cells (and atoms position near the current cells
			  //          2) compute interactions with P2Premote method (no mutual interactions)
			  //     For non periodic interactions we proceed with mutual algorithm (classical P2P)
			  //
...
#pragma omp master
    ...
    if(p2pEnabled && nbProcess > 1 ){
        ...
        int indexArray[26];
        int neighborPositions[26];
        // Box limite in terms of cell by direction
...
#pragma omp for  schedule(dynamic, userChunkSize)
        for(int idxLeafs = 0 ; idxLeafs < nbLeafToProceed ; ++idxLeafs){
            ...
            // Take all possible neighbors of cell currentIter.coord (26 cells)
...
public:
  ...
  ///
  ///
  /// @param[in]  center FTreeCoordinate of the currentcell
  /// @param[in]  inDirection  periodic firection (enum)
  /// @param[out] indexes the morton index of all adjacents cells
  /// @param[out] indexInArray --
  ///
  /// @result return the number of neighbors (26 ?)
  int getNeighborsIndexesPeriodic(const FTreeCoordinate& center, const int& boxLimite,
                                  MortonIndex indexes[26],
  int indexInArray[26], const int &inDirection) const{
    if(center.getX() != 0 && center.getY() != 0 && center.getZ() != 0 &&
       center.getX() != boxLimite -1 && center.getY() != boxLimite -1  && center.getZ() != boxLimite -1 ){
        return this->getNeighborsIndexes(center, indexes, indexInArray);
      }
  ...
  int getNeighborsIndexes(const FTreeCoordinate& center, MortonIndex indexes[26],
  int indexInArray[26]) const{
    int idxNeig = 0;
  ...
  int getPeriodicInteractionNeighbors(const FTreeCoordinate& workingCell,const int inLevel, MortonIndex inNeighbors[/*189+26+1*/216],
  int inNeighborsPosition[/*189+26+1*/216],
  ...
  int getInteractionNeighbors(const FTreeCoordinate& workingCell,const int inLevel,
                              MortonIndex inNeighbors[/*189+26+1*/216],
  int inNeighborsPosition[/*189+26+1*/216],
