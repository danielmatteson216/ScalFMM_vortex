...
struct FAlignedAllocator {
    ...
    struct rebind {
        using other = FAlignedAllocator<Alignment, U, Us...>;
    };
public:
     ...
     */
    pointer_tuple allocate(std::size_t n) {
        // allocated storage
        ...
        char** offset_ptr = reinterpret_cast<char**>(
            reinterpret_cast<char*>(std::get<0>(ptr_tuple)) - sizeof(void*));
...
private:
    ...
    bool align_ptr(alignment_arguments& args, pointer_tuple& ptr_tuple, inria::index_sequence<Is...>) {

        #ifdef DEBUG_ALLOC
        { // DEBUG print
            std::cerr << std::setw(10) << "type"
                      << std::setw( 7) << "sizeof"
                      << std::setw(12) << "origin"
                      << std::setw(12) << "free space"
                      << std::setw(12) << "array size"
                      << std::setw(12) << "offset"
                      << std::setw(12) << "final"
                      << std::setw(12) << "check"
                      << std::setw(12) << "next"
                      << std::setw(12) << "rem. space"
                      << std::endl;
        }
        #endif
     ...
     */
    template<typename U>
    bool align_ptr_impl(alignment_arguments& args, U*& res_ptr) {
        ...
        { // DEBUG print
            ...
            std::cerr << std::setw(10) << typeid(U).name()
                      << std::setw( 7) << sizeof(U)
                      << std::setw(12) << args.ptr
                      << std::setw(12) << args.ptr_space
                      << std::setw(12) << ptr_size
                ;
        }
        #endif
     ...
     */
    template<typename T>
    std::size_t type_array_size(std::size_t n) {
        // Length of the array
    ...
    static_assert(sizeof...(Ts) > 0, "FAlignedAllocator needs at least one type.");
};
...
struct FAlignedAllocator<Alignment, T> {
    ...
    pointer allocate(std::size_t n) {
        ...
        // Get aligned pointer
        if(! std::align(Alignment, ptr_size, ptr, ptr_space) ) {
            free(buffer);
            throw std::bad_alloc();
        }
    ...
    bool operator==(const FAlignedAllocator<AlignU, U>& /*other*/) const {
