 ...
 *
 * The @class FDft is templatized with the input value type (FReal or stdComplex<FReal>),
...
class FDft
{
    ...
    stdComplex<FReal>* dataF_; //< data in Fourier space
...
private:
    ...
    void initDFT()
    {
        ...
        dataF_ = new stdComplex<FReal>[size_];
...
public:
    ...
    void applyDFT(const FReal* sampledData,
                  stdComplex<FReal>* transformedData) const
    {
        // read sampled data
        ...
        for(unsigned int r=0; r<size_; ++r)
            for(unsigned int s=0; s<size_; ++s){
                dataF_[r] += stdComplex<FReal>(data_[s]*cosRS_[r*size_+s],
    ...
    // Complexe valued DFT
    void applyDFT(const stdComplex<FReal>* sampledData,
                  stdComplex<FReal>* transformedData) const
    {
        // read sampled data
        FBlas::c_setzero(size_,reinterpret_cast<FReal*>(dataF_));
        ...
        for(unsigned int r=0; r<size_; ++r)
            for(unsigned int s=0; s<size_; ++s){
                dataF_[r] += stdComplex<FReal>(data_[s].getReal()*cosRS_[r*size_+s]
    ...
    /// Backward DFT
    // Real valued IDFT
    void applyIDFT(const stdComplex<FReal>* transformedData,
    ...
    void applyIDFT(const stdComplex<FReal>* transformedData,
                   stdComplex<FReal>* sampledData) const
    {
        // read transformed data
        ...
        for(unsigned int r=0; r<size_; ++r){
            for(unsigned int s=0; s<size_; ++s){
                data_[r] += stdComplex<FReal>(dataF_[s].getReal()*cosRS_[r*size_+s]
                        - dataF_[s].getImag()*sinRS_[r*size_+s],
...
protected:
    ...
    static void PlusEqual(stdComplex<double> dest[], const fftw_complex src[], const int nbElements){
        const double* ptrSrc = reinterpret_cast<const double*>(src);
        for(int idxVal = 0 ; idxVal < nbElements ; ++idxVal){
            dest[idxVal] += stdComplex<double>(ptrSrc[2*idxVal], ptrSrc[2*idxVal+1]);
        }
    }
    ...
    static void Equal(stdComplex<double> dest[], const fftw_complex src[], const int nbElements){
        const double* ptrSrc = reinterpret_cast<const double*>(src);
        for(int idxVal = 0 ; idxVal < nbElements ; ++idxVal){
            dest[idxVal] = stdComplex<double>(ptrSrc[2*idxVal], ptrSrc[2*idxVal+1]);
        }
    ...
    static void PlusEqual(stdComplex<float> dest[], const fftwf_complex src[], const int nbElements){
        const float* ptrSrc = reinterpret_cast<const float*>(src);
        for(int idxVal = 0 ; idxVal < nbElements ; ++idxVal){
            dest[idxVal] += stdComplex<float>(ptrSrc[2*idxVal], ptrSrc[2*idxVal+1]);
        }
    }
    ...
    static void Equal(stdComplex<float> dest[], const fftwf_complex src[], const int nbElements){
        const float* ptrSrc = reinterpret_cast<const float*>(src);
        for(int idxVal = 0 ; idxVal < nbElements ; ++idxVal){
            dest[idxVal] = stdComplex<float>(ptrSrc[2*idxVal], ptrSrc[2*idxVal+1]);
        }
...
public:
    ...
    */
    ...
    void normalize(stdComplex<double>* resultSignal) const {
        const double realNbPoints = static_cast<double>(nbPoints);
        for(int idxVal = 0 ; idxVal < nbPoints ; ++idxVal){
            resultSignal[idxVal] /= realNbPoints;
        }
    ...
    void normalize(stdComplex<float>* resultSignal) const {
        const float realNbPoints = static_cast<float>(nbPoints);
        for(int idxVal = 0 ; idxVal < nbPoints ; ++idxVal){
            resultSignal[idxVal] /= realNbPoints;
        }
...
//////////////////////////////////////////////////////////////////////////////
template <int DIM>
class FFftw <double, stdComplex<double>, DIM> : public FFftwCore<double, stdComplex<double>, double, fftw_complex, fftw_plan, DIM>{
    typedef FFftwCore<double, stdComplex<double>, double, fftw_complex, fftw_plan, DIM> ParentClass;
public:
    using ParentClass::ParentClass;
};
template <int DIM>
class FFftw <stdComplex<double>, stdComplex<double>, DIM> : public FFftwCore<stdComplex<double>, stdComplex<double>, fftw_complex, fftw_complex, fftw_plan, DIM>{
    typedef FFftwCore<stdComplex<double>, stdComplex<double>, fftw_complex, fftw_complex, fftw_plan, DIM> ParentClass;
public:
    using ParentClass::ParentClass;
};
//////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////
template <int DIM>
class FFftw <float, stdComplex<float>, DIM> : public FFftwCore<float, stdComplex<float>, float, fftwf_complex, fftwf_plan, DIM>{
    typedef FFftwCore<float, stdComplex<float>, float, fftwf_complex, fftwf_plan, DIM> ParentClass;
public:
    using ParentClass::ParentClass;
};
template <int DIM>
class FFftw <stdComplex<float>, stdComplex<float>, DIM> : public FFftwCore<stdComplex<float>, stdComplex<float>, fftwf_complex, fftwf_complex, fftwf_plan, DIM>{
    typedef FFftwCore<stdComplex<float>, stdComplex<float>, fftwf_complex, fftwf_complex, fftwf_plan, DIM> ParentClass;
public:
    using ParentClass::ParentClass;
};
